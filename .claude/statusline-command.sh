...
# If used_percentage is 0, try remaining_percentage, otherwise show "?"
# NOTE: Cumulative tokens / context_max is WRONG - that's not current context fill
# CC v2.1.6+ provides used_percentage; older versions need upgrade for accuracy
if [ "$context_pct" = "0" ]; then
    if [ "$context_remaining" != "100" ] && [ "$context_remaining" != "0" ]; then
        # remaining_percentage is available, calculate from it
        context_pct=$((100 - context_remaining))
    else
        # No accurate data available - mark as unknown
        context_pct="?"
    fi
fi
...
# Extract additional data from JSON (context_pct already calculated above with fallback)
eval "$(echo "$input" | jq -r '
  "current_dir=" + (.workspace.current_dir // .cwd | @sh) + "\n" +
...
render_context_bar() {
    ...
    # Handle unknown percentage (show empty bar)
    [ "$pct" = "?" ] && pct=0
...
# Handle unknown context percentage (CC version without used_percentage)
if [ "$raw_pct" = "?" ]; then
    display_pct="?"
    pct_color="$SLATE_400"  # Muted color for unknown
# Scale percentage: if threshold is 62, then 62% raw = 100% displayed
# Formula: display_pct = (raw_pct * 100) / threshold
elif [ "$COMPACTION_THRESHOLD" -lt 100 ] && [ "$COMPACTION_THRESHOLD" -gt 0 ]; then
...
# Skip if display_pct is "?" (already colored above)
if [ "$display_pct" != "?" ]; then
